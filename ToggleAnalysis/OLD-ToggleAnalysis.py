...
def run(context):
    try:
        ...
        try:
            ...
            # Fallback: try the Utility panel in Tools tab
            try:
                designWorkspace = ui.workspaces.itemById('FusionSolidEnvironment')
                if designWorkspace:
                    toolsTab = next((tab for tab in designWorkspace.toolbarTabs if tab.id == 'ToolsTab'), None)
                    if toolsTab:
                        utilityPanel = next((panel for panel in toolsTab.toolbarPanels if panel.id == 'UtilityPanel'), None)
                        if utilityPanel:
                            cmdControl = utilityPanel.controls.addCommand(cmdDef)
                            cmdControl.isVisible = True
            except:
                # Final fallback: Quick Access Toolbar
                try:
                    qat = ui.toolbars.itemById('QAT')
                    if qat:
                        cmdControl = qat.controls.addCommand(cmdDef)
                        cmdControl.isVisible = True
                except:
                    pass
